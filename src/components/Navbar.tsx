...
interface Notification {
  id: number          // FIX: was `string` — DB uses Int @id
  type: string
  title: string
  message: string
  createdAt: string   // FIX: API returns `createdAt`, not `time`
  read: boolean
}
...
export default function Navbar() {
  ...
  const [notifications, setNotifications] = useState<Notification[]>([])
  const [showNotifications, setShowNotifications] = useState(false)
  const [readIds, setReadIds] = useState<Set<number>>(new Set())  // FIX: Set<number> to match Int IDs
  const desktopNotifRef = useRef<HTMLDivElement>(null)  // FIX: split into two refs (was one ref used on two elements)
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        if (!res.ok) return
        const data = await res.json()
        // FIX: API returns { notifications: [...], unreadCount: N }
        // The old code did setNotifications(data) — setting an object, not the array
        const notifArray: Notification[] = Array.isArray(data.notifications)
          ? data.notifications
          : Array.isArray(data)
            ? data
            : []
        ...
        setReadIds(new Set<number>(storedIds))
  ...
  // FIX: outside-click now checks both desktop and mobile refs
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      const target = e.target as Node
      const insideDesktop = desktopNotifRef.current?.contains(target)
    ...
    <nav className="bg-white shadow-sm text-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
                    ...
                    <div className="overflow-y-auto flex-1">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map(n => (
                          <div
                            key={n.id}
                            className={`px-4 py-3 border-b last:border-b-0 ${readIds.has(n.id) ? 'bg-white' : 'bg-blue-50'}`}
                          >
                            <div className="flex gap-2">
                              <span className="text-base mt-0.5">{getNotifIcon(n.type)}</span>
                              <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center sm:hidden">
            {isAuthenticated && !isAdmin && (
              <div className="relative mr-2" ref={mobileNotifRef}>
                <button onClick={() => setShowNotifications(!showNotifications)} className="relative p-2 text-gray-500">
                  <FaBell className="h-5 w-5" />
                  {unreadCount > 0 && (
                    <span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center font-bold">
